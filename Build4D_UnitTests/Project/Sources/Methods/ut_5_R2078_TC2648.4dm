...
// build the client application with the "Use SDI in interface" settings set to true
...
If ($infoPlist.exists)
	...
	ASSERT($infos["SDIRuntime"]="1"; "(Current project) Info.plist SDIRuntime Key should have value: 1")
...
If ($infoPlist.exists)
	...
	ASSERT($infos["SDIRuntime"]="1"; "(External project) Info.plist SDIRuntime Key should have value: 1")
