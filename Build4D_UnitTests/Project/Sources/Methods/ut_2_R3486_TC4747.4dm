//%attributes = {}
// Test the component signature with custom signature settings (True, None, False) in specified destination folder
If (Is macOS)
	...
	ASSERT($success=False; "(Current project) Component build should fail"+$link)  // No certificate name, AdHoc signature is performed only if macSignature = False
	...
	ASSERT($success=False; "(External project) Component build should fail"+$link)  // No certificate name, AdHoc signature is performed only if macSignature = False
