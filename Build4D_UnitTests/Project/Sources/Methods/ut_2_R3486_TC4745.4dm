//%attributes = {}
// Test the component signature with custom signature settings (True, Unknown certificate, None) in specified destination folder
If (Is macOS)
	...
	$settings.signApplication:=New object(\
		"macSignature"; True; \
		"macCertificate"; "WrongCertificateName")
	
	$build:=cs.Build4D.Component.new($settings)
	
	$success:=$build.build()
	
	ASSERT($success=False; "(Current project) Component build should fail"+$link)  // Wrong certificate name
	...
	ASSERT($success=False; "(External project) Component build should fail"+$link)  // Wrong certificate name
