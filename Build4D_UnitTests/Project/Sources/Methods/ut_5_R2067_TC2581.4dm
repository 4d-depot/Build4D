//%attributes = {}
// build the client application with a specific "buildName"
...
$settings.buildName:="Specific Build Name"
...
If ($infoPlist.exists)
	...
	ASSERT($infos["BuildName"]=$settings.buildName; "(Current project) Info.plist BuildName Key should have value: "+$settings.buildName)
Else 
...
If ($infoPlist.exists)
	$infos:=$infoPlist.getAppInfo()
	ASSERT($infos["BuildName"]=$settings.buildName; "(External project) Info.plist BuildName Key should have value: "+$settings.buildName)
Else 
