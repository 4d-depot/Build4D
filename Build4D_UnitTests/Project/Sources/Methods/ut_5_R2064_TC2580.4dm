...
// integrate the archive client into the server, then check that the info.Json file is created in the Uprade4DClient
...
If (Is macOS)
	$clientArchive:=$build.settings.destinationFolder.file("Contents/Upgrade4DClient/info.json")
Else 
	// to validate on windows
	$clientArchive:=$build.settings.destinationFolder.file("Upgrade4DClient/info.json")
End if 
